## General
This project uses .NET Core 9.0 and C# 13.0. Best practices should be followed using modern constructs where applicable.
...
- Test methods should be named `MethodName_ShouldExpectedResult_WhenConditionOccurs`; if the method has an expected return value the names should be `MethodName_ShouldReturnExpectedValue_WhenConditionOccurs`.
- If the method under test is overloaded, the test method Should include the parameter types in the name, e.g., `MethodName_WithType_ShouldExpectedResult_WhenConditionOccurs`.
