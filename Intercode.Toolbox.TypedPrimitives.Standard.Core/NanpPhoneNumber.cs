...
namespace Intercode.Toolbox.TypedPrimitives.Standard.Core;

using System.Text.RegularExpressions;
using FluentResults;

[TypedPrimitive<string>]
public readonly partial struct NanpPhoneNumber
{
  #region Constants

  private const int DEFAULT_REGEX_TIMEOUT_IN_MS = 200;
  private const string DEFAULT_PATTERN = @"^(\+?1)?[-.\s]?(\()?(\d{3})(\))?[-.\s]?(\d{3})[-.\s]?(\d{4})$";
    ...
    ref Result result )
  {
    if( value is null )
    {
      result = Result.Fail( $"A {nameof( NanpPhoneNumber )} cannot be null" );
      return;
    }

    if( !CreateDefaultRegex().IsMatch( value ) )
    {
      result = Result.Fail( $"Invalid {nameof( NanpPhoneNumber )} format" );
    }
